 ...
 * 仿qq消息长按菜单弹窗
 */
public class UCULQPopupWindow extends PopupWindow
{
    ...
    private static UCULQPopupWindow popupList = null;
    private ColorStateList mTextColorStateList;
    private int mRawX;
    private int mRawY;

    private UCULQPopupWindow(Context context)
    {
        super(context);
    ...
    public static synchronized UCULQPopupWindow getInstance(Context context)
    {
        if (popupList == null)
        {
            popupList = new UCULQPopupWindow(context);
        }
        return popupList;
    }
    ...
    public class Builder
    {
         ...
         * item 单元点击回调
         */
        public Builder setOnPopupListItemClickListener(OnPopupListItemClickListener listener)
        {
        ...
        public void show()
        {
            ...
            if (config.mPopupWindow == null)
            {
                ...
                {
                    throw new NullPointerException("UCULLongPressMenu item dataSources is null,please make sure (Builder)setPopupItemList() invoked");
                }
                else
                {
            ...
            for (int i = 0; i < config.mPopupItemList.size(); i++)
            {
                ...
                textView.setOnClickListener(new View.OnClickListener()
                {
                    ...
                    public void onClick(View v)
                    {
                        ...
                        {
                            config.mListener.onPopupListItemClick(config.mAnchorView, config.position, finalI);
                        }
        ...
        private void checkAnchorView()
        {
            ...
            {
                throw new NullPointerException("UCULLongPressMenu AnchorView is null,please make sure (Builder)bindView() invoked");
            }
        }
    }
    ...
    public interface OnPopupListItemClickListener
    {
         ...
         * @param position           为 pop 中的点击菜单 item 的 position ，第一个位置索引为 0，以此类推
         */
        void onPopupListItemClick(View anchorView, int anchorViewPosition, int position);
    }
}
